class YouTubeSubtitleOverlay {
  ...
  init() {
    ...
    // Listen for auto-load trigger from transcript-core.js
    window.addEventListener('YTSP_TriggerAutoLoad', (e) => {
      console.log('Received auto-load trigger for:', e.detail?.videoId);
      this.autoLoadEnabled = true;
      this.attemptAutoLoad();
    });

    // Listen for translation trigger from transcript-core.js
    window.addEventListener('YTSP_StartTranslation', () => {
      console.log('🚀 Received translation trigger from page button');
      // 通知 background 启动翻译流程
      this.startTranslationFromPage();
    });
  }

  async startTranslationFromPage() {
    try {
      // 获取字幕
      ...
      console.log('✅ 已发送翻译请求到后台:', subtitles.length, '条字幕');
    } catch (error) {
  ...
  /**
   * 获取YouTube官方字幕 (直接从Transcript面板获取)
   * @returns {Promise<Array<{startTime: number, endTime: number, text: string}>>}
   */
  async fetchYouTubeOfficialSubtitles() {
    const videoId = this.getVideoId();
    if (!videoId) {
   ...
   * 从Transcript面板获取字幕
   */
  async getSubtitlesFromTranscriptPanel() {
    // 先检查Transcript面板是否已经打开
    ...
    transcriptSegments.forEach((segment, index) => {
      ...
      if (timestampElement && textElement) {
        ...
        if (text) {
          // 估算结束时间（下一条字幕的开始时间或当前+3秒）
          const nextSegment = transcriptSegments[index + 1];
          let endTime = timestamp + 3;
          if (nextSegment) {
            const nextTimestamp = nextSegment.querySelector('.segment-timestamp') ||
    ...
    if (subtitles.length > 0) {
      console.log('✅ 从Transcript面板获取到', subtitles.length, '条字幕');
    }
  ...
  updateSubtitle() {
    if (!this.currentVideo || !this.isEnabled || !this.overlayElement) {
      console.log('⚠️ updateSubtitle 跳过:', {
  ...
  loadBilingualSubtitles(englishSubtitles, chineseSubtitles) {
    console.log('📥 loadBilingualSubtitles 接收到字幕:',
      englishSubtitles?.length || 0, '条英文,',
      chineseSubtitles?.length || 0, '条中文');
    ...
    if (this.englishSubtitles.length > 0 || this.chineseSubtitles.length > 0) {
      ...
      // 确保 video 和 overlay 存在
      if (!this.currentVideo) {
        this.currentVideo = document.querySelector('video');
        console.log('🎬 获取视频元素:', !!this.currentVideo);
      }
      ...
      console.log('✅ 字幕已启用, isEnabled =', this.isEnabled,
        ', hasVideo =', !!this.currentVideo,
        ', hasOverlay =', !!this.overlayElement);
      ...
      if (this.currentVideo && this.overlayElement) {
        this.setupVideoListeners(); // 确保 timeupdate 监听器存在
        this.updateSubtitle();
      }
    }
  }
  ...
  async loadSubtitleData() {
    try {
      ...
      // 🔧 修复: 如果有缓存的字幕数据且字幕已启用,立即显示
      const hasSubtitles = this.englishSubtitles.length > 0 ||
        this.chineseSubtitles.length > 0 ||
        this.subtitleData.length > 0;

      if (hasSubtitles && this.isEnabled) {
        console.log('📦 从缓存加载字幕:', this.englishSubtitles.length, '条英文,', this.chineseSubtitles.length, '条中文');
  ...
  async attemptAutoLoad() {
    ...
    if (!isNewVideo && hasExistingSubtitles) {
      return;
    }
