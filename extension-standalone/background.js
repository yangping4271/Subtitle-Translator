...
// 导入统一配置中心和翻译服务
importScripts('config.js', 'translator.js');
...
class SubtitleExtensionBackground {
    ...
    try {
      switch (request.action) {
        ...
        case 'getTranslationStatus':
          ...
          sendResponse({ success: true, progress: status.translationProgress || null });
          break;
  ...
  async saveVideoSubtitles(videoId, englishSubtitles, chineseSubtitles, subtitleData, englishFileName, chineseFileName, fileName) {
    ...
    // 🔧 修复：保存后立即通知content.js加载字幕
    if (englishSubtitles || chineseSubtitles) {
      await this.notifyContentScript('loadBilingualSubtitles', {
        englishSubtitles: englishSubtitles || [],
        chineseSubtitles: chineseSubtitles || []
      });
      console.log('✅ 已通知content.js加载字幕:', (englishSubtitles?.length || 0), '条英文,', (chineseSubtitles?.length || 0), '条中文');
    }
  }
   ...
   * 在后台启动翻译任务
   */
  async startBackgroundTranslation(request, sendResponse) {
    ...
    console.log('🚀 后台开始翻译:', subtitles.length, '条字幕');
    ...
    try {
      ...
      console.log('✅ 后台翻译完成:', result.chinese.length, '条中文字幕');
   ...
   * 取消后台翻译
   */
  async cancelBackgroundTranslation(sendResponse) {
