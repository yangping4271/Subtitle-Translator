// 调试版本的content.js - 添加更多日志输出
class YouTubeSubtitleTranslator {
  constructor() {
    console.log('🚀 YouTube双语字幕翻译器开始初始化...');
  ...
  async init() {
    ...
    console.log('⚙️ 当前设置:', this.settings);
    
    // 检查API密钥
    if (!this.settings.apiKey) {
      console.error('❌ 未配置API密钥！请在插件设置中配置。');
      this.showDebugInfo('未配置API密钥！请点击插件图标配置。');
      return;
    }
    
    // 创建翻译显示容器
    console.log('🎨 创建翻译显示容器...');
    this.createTranslationContainer();
    
    // 开始监听字幕变化
    console.log('👁️ 开始监听字幕变化...');
    ...
    console.log('✅ YouTube双语字幕翻译器初始化完成');
    this.showDebugInfo('插件已启动，等待检测字幕...');
    
    // 立即检查一次字幕
    setTimeout(() => {
      console.log('🔍 执行首次字幕检查...');
  ...
  createTranslationContainer() {
    ...
    if (!playerContainer) {
      console.error('❌ 未找到YouTube播放器容器，3秒后重试...');
  ...
  startSubtitleMonitoring() {
    ...
    const startObserving = () => {
      ...
      console.log(`👀 开始观察 ${observedCount} 个字幕元素`);
  ...
  async checkSubtitleChange() {
    if (!this.settings.autoTranslate || !this.settings.apiKey) {
      console.log('⏸️ 翻译已暂停（autoTranslate=' + this.settings.autoTranslate + ', hasApiKey=' + !!this.settings.apiKey + ')');
      return;
    }
    ...
    console.log('📝 当前检测到的字幕:', currentSubtitle);
    ...
    for (const selector of selectors) {
      ...
      for (const element of elements) {
        if (element && element.textContent && element.textContent.trim()) {
          ...
          console.log(`✅ 从 "${selector}" 获取到字幕:`, text);
          return text;
        }
      }
    }
    
    console.log('❌ 未检测到任何字幕');
    return null;
  }
  
  async translateSubtitle(text) {
    if (this.isTranslating) {
      console.log('🔄 翻译进行中，跳过...');
      return;
    }
    ...
    if (this.translationCache.has(text)) {
      ...
      console.log('💾 使用缓存翻译:', translation);
...
请翻译以下字幕：`;
  ...
  updateSubtitleDisplay(original, translated) {
    ...
    if (original || translated) {
      ...
      console.log('📺 显示字幕:', { original, translated });
    } else {
